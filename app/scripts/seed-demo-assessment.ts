...
async function seed() {
  ...
  const { error: globalInputsError } = await supabase
    ...
    .insert({
      ...
      current_marketing_maturity: 2,
      existing_tech_stack: 'Salesforce Marketing Cloud Engagement, Commerce Cloud, Google Analytics 4, Loyalty Lion, Braze (SMS)',
      team_size: '25-50 person marketing team',
      annual_marketing_budget: '$10M–$25M',
      preferred_engagement_models: ['implementation', 'advisory'],
      budget_range: '$500K–$1M',
      budget_flexibility: 'Moderate — tied to FY budget cycle',
      decision_timeline: '60-90 days',
      internal_resources: 'Strong internal team, limited data engineering capacity',
      prefer_capex_or_opex: 'OpEx preferred',
      key_business_drivers: ['Increase repeat purchase rate', 'Reduce churn among loyalty members', 'Improve email ROI with personalization'],
      success_metrics: ['Email revenue lift', 'Customer LTV growth', 'Loyalty enrollment rate'],
      known_constraints: 'Legacy POS system not yet integrated with marketing. Privacy team requires consent review before any new data collection.',
      competitive_pressures: 'Nike and Adidas direct-to-consumer push is pulling customers away from multi-brand retail. Need to strengthen loyalty and personalization.',
      executive_sponsor: 'SVP of Marketing & Digital',
      additional_context: 'Foot Locker recently launched a new loyalty program (FLX Rewards) and wants to drive adoption and engagement through personalized journeys.',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    });
